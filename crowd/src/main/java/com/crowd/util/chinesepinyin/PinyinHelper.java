package com.crowd.util.chinesepinyin;
...
public final class PinyinHelper {
     ...
     * 
     * @param str 需要转换的字符串
     * @param separator 拼音分隔符   如果分隔符是Upper 则拼音首字母大写
     * @param pinyinFormat 拼音格式：WITH_TONE_NUMBER--数字代表声调，WITHOUT_TONE--不带声调，WITH_TONE_MARK--带声调
     * @return 字符串的拼音
     */
            ...
            if (ChineseHelper.isChinese(c) || c == CHINESE_LING) {
                ...
                for (int rightIndex = (i + rightMove) < len ? (i + rightMove) : (len - 1); rightIndex > i; rightIndex--) {
                    ...
                    if (MUTIL_PINYIN_TABLE.containsKey(cizu)) {
                        ...
                        for (int j = 0, l = pinyinArray.length; j < l; j++) {
                        	/*如果分隔符是Upper则拼音首字母大写*/
                        	if("Upper".equals(separator)){
                        		sb.append(toUpperFristChar(pinyinArray[j]));
                                if (j < l - 1) {
                                    sb.append("");
                                }
                        	}else{
                ...
                if("Upper".equals(separator)){
                	if (!isFoundFlag) {
                        ...
                        if (pinyinArray != null) {
                        	/*如果分隔符是Upper则拼音首字母大写*/
                            sb.append(toUpperFristChar(pinyinArray[0]));
                        } else {
    ...
    /**
	 * 字符串第一位小写字母转换成大写
	 * 字符串第一位是是小写字母可以转换，如果字符串第一位不是小写字母，不做任何处理返回。
	 * @param str 待转换的字母字符串
	 * @return 返回首字母大写的字母字符串
	 */
    public static String toUpperFristChar(String str) {
    	char[] charArray = str.toCharArray();
    	Pattern pattern = Pattern.compile("[a-z]*"); 
    	Matcher matcher = pattern.matcher(charArray[0]+"");
    		if(matcher.matches()){
    			charArray[0] -= 32;
    		}
    	return String.valueOf(charArray);
    ...
    /**
     * 小写字母转大写
     * 字符是小写字母返回对应大写字母，若不是则不做任何处理返回。
     * @param c 待转换的小写字母字符
     * @return   返回 大写的字母字符
     */
    public static String toUpperChar(char c) {
    	Pattern pattern = Pattern.compile("[a-z]*");  
